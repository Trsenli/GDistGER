...
#define checkCUDAerr(err)                                                  \
  {                                                                        \
    ...
    if (cudaSuccess != cet)                                                \
    {                                                                      \
      printf("[ %d ]%s %d : %s\n",my_rank, __FILE__, __LINE__, cudaGetErrorString(cet)); \
      exit(0);                                                             \
    }                                                                      \
  }
...
__global__ void __old_sgNegReuse(const int window, const int layer1_size, const int negative, const int vocab_size, float alpha,
                             ...
                             float *syn1, float *syn0, const int *negSample)
{
  ...
  const int tid = threadIdx.x + blockDim.x * threadIdx.y; // thread ID (in global)
  const int dxy = blockDim.x * blockDim.y;                // block 里面有多少线程，一个负样本的计算能给32个线程
  // ========= Load Buffer ==============
  // 一个block 对应两个句子
  ...
  for (int sentPos = sentIdx_s; sentPos < sentIdx_e; sentPos++) {       // 遍历句子中的词
    ...
    for (int a = 0; a < window * 2 + 1; a++) // 遍历窗口
      ...
      {                               // 对于每个 context word
        ...
        if (c >= tmp_sent_s && c < tmp_sent_e && sen[c] != -1)
        {
          ...
          // 定位target 的向量
          // l2 = target * layer1_size; // 对于一个上下文词来说，target 要么是中心词就是正样本 ，如果是负样本词，就是负样本。
          l2 = getNegBufIdx(target) * layer1_size;

          // 每个上下文词与负样本做运算
          // 梯度计算

          for (int i = threadIdx.x; i < layer1_size; i += blockDim.x)
          { // Get gradient
            // f += syn0[i + l1] * syn1[i + l2];
          ...
          if (hs)
            for (int d = vocab_codelen[word]; d < vocab_codelen[word + 1]; d++)
            {
              ...
              if (threadIdx.x < FSIZE)
                f[threadIdx.x] = syn0[threadIdx.x + l1] * syn1[threadIdx.x + l2];
              __syncthreads();
              if (threadIdx.x >= FSIZE)
                f[threadIdx.x % (FSIZE)] += syn0[threadIdx.x + l1] * syn1[threadIdx.x + l2];
              __syncthreads();
...
{ // 提前生成负样本的采样表
  ...
  // FOR CUDA
  checkCUDAerr(cudaMalloc((void **)&d_table, table_size * sizeof(int))); // 拷贝到GPU里
...
void InitNet()
{
  ...
  checkCUDAerr(cudaHostAlloc((void **)&syn0, (long long)vocab_size * layer1_size * sizeof(float) ,cudaHostAllocWriteCombined | cudaHostAllocMapped));
  if (syn0 == NULL)
  {
  ...
  { // ==== 使用负采样  syn1neg 应该对应的 woh
    ...
    checkCUDAerr(cudaHostAlloc((void **)&syn1neg, (long long)vocab_size * layer1_size * sizeof(float) ,cudaHostAllocWriteCombined | cudaHostAllocMapped));
    if (syn1neg == NULL)
    {
      printf("Memory allocation failed\n");
      exit(1);
...
void cbowKernel(int *d_sen, int *d_sent_len, float alpha, int cnt_sentence, int reduSize)
{
  ...
  {
  case 128:
    cbow_kernel<64><<<gDim, bDim>>>(window, layer1_size, negative, hs, table_size, vocab_size, alpha,
                                    d_expTable, d_table, d_vocab_codelen, d_vocab_point, d_vocab_code,
                                    d_sen, d_sent_len, d_syn1, d_syn0);
    break;
  case 256:
    cbow_kernel<128><<<gDim, bDim>>>(window, layer1_size, negative, hs, table_size, vocab_size, alpha,
                                     d_expTable, d_table, d_vocab_codelen, d_vocab_point, d_vocab_code,
                                     d_sen, d_sent_len, d_syn1, d_syn0);
    break;
  case 512:
    cbow_kernel<256><<<gDim, bDim>>>(window, layer1_size, negative, hs, table_size, vocab_size, alpha,
                                     d_expTable, d_table, d_vocab_codelen, d_vocab_point, d_vocab_code,
                                     d_sen, d_sent_len, d_syn1, d_syn0);
    break;
  default:
...
void sgKernel(int *d_sen, int *d_sent_len, int *d_negSample, float alpha, int cnt_sentence, int reduSize)
{
  ...
  {                                    // A sentence share negative samples
    ...
    case 128:
      ...
      break;
    case 64:
  __sgNegReuse<64><<<gDim, bDimNeg>>> 
      (window, layer1_size, negative, vocab_size, alpha,
        d_sen, d_sent_len, d_syn1, d_syn0, d_negSample);
  break;
    case 200:
      __sgNegReuse<200><<<gDim, bDimNeg>>>(window, layer1_size, negative, vocab_size, alpha,
                                           d_sen, d_sent_len, d_syn1, d_syn0, d_negSample);
      break;
    case 300:
  ...
  else
  {
    ...
    {
    case 128:
      skip_gram_kernel<64><<<gDim, bDim>>>(window, layer1_size, negative, hs, table_size, vocab_size, alpha,
                                           d_expTable, d_table, d_vocab_codelen, d_vocab_point, d_vocab_code,
                                           d_sen, d_sent_len, d_syn1, d_syn0);
      break;
    case 256:
      skip_gram_kernel<128><<<gDim, bDim>>>(window, layer1_size, negative, hs, table_size, vocab_size, alpha,
                                            d_expTable, d_table, d_vocab_codelen, d_vocab_point, d_vocab_code,
                                            d_sen, d_sent_len, d_syn1, d_syn0);
      break;
    case 512:
      skip_gram_kernel<256><<<gDim, bDim>>>(window, layer1_size, negative, hs, table_size, vocab_size, alpha,
                                            d_expTable, d_table, d_vocab_codelen, d_vocab_point, d_vocab_code,
                                            d_sen, d_sent_len, d_syn1, d_syn0);
      break;
    default:
        ...
        if (sync_eclipsed > model_sync_period)
        {
          ...
          if ((full_sync_times > 0) && (progress > (real)full_sync_count / (full_sync_times + 1) + 0.01f))
          {
            ...
            for (int r = 0; r < num_rounds; r++)
            {
              ...
              MPI_Allreduce(MPI_IN_PLACE, syn0 + start * layer1_size, sync_size * layer1_size, MPI_SCALAR, MPI_SUM, MPI_COMM_WORLD);
              MPI_Allreduce(MPI_IN_PLACE, syn1neg + start * layer1_size, sync_size * layer1_size, MPI_SCALAR, MPI_SUM, MPI_COMM_WORLD);
...
#pragma vector aligned
          ...
          {
            real *sync_block_in = (real *)_mm_malloc((size_t)vocab_size * layer1_size * sizeof(real), 64);
            real *sync_block_out = (real *)_mm_malloc((size_t)vocab_size * layer1_size * sizeof(real), 64);
            vector<int> sync_ind;
            ...
            MPI_Bcast(sync_ind.data(), sync_block_size, MPI_INT, 0, MPI_COMM_WORLD);

            for (size_t i = 0; i < sync_block_size; i++)
            {
              // printf("id %d, sync_id %d\n",i, sync_ind[i]);

              memcpy(sync_block_in + i * layer1_size, syn0 + (size_t)sync_ind[i] * layer1_size, layer1_size * sizeof(real));
              memcpy(sync_block_out + i * layer1_size, syn1neg + (size_t)sync_ind[i] * layer1_size, layer1_size * sizeof(real));
            }

            MPI_Allreduce(MPI_IN_PLACE, sync_block_in, sync_block_size * layer1_size, MPI_SCALAR, MPI_SUM, MPI_COMM_WORLD);
            MPI_Allreduce(MPI_IN_PLACE, sync_block_out, sync_block_size * layer1_size, MPI_SCALAR, MPI_SUM, MPI_COMM_WORLD);
...
#pragma vector aligneds
    ...
    else
    {
      ...
      checkCUDAerr(cudaMalloc((void **)&d_sent_len, (1 + MAX_SENTENCE) * sizeof(int)));
      //  checkCUDAerr(cudaHostGetDevicePointer(&d_sent_len, sentence_length, 0));
        //  int *negSample;
      int *negSample = (int *)malloc(MAX_SENTENCE * negative * sizeof(int));
      // checkCUDAerr(cudaHostAlloc((void **)&negSample,MAX_SENTENCE * negative * sizeof(int),cudaHostAllocWriteCombined | cudaHostAllocMapped));
      int *d_negSample;
