...
template<typename T>
void prl_read_edge_by_txt(const char* fname, Edge<T>*& edge, size_t& e_num,int threads)
{
...
#pragma omp parallel num_threads(threads)
    {
        ...
        size_t thread_edge_num = e_num / threads + e_num % threads > 0 ? 1 : 0;
        size_t edge_start = id * thread_edge_num;
...
template<typename T>
void prl_read_edge_by_bin(const char* fname, Edge<T>* & edge, size_t& g_e_num,int partition_num)
{
	FILE *f = fopen(fname,"rb");
        assert(f != NULL);
        fseek(f,0, SEEK_END);
        size_t total_size = ftell(f);
        g_e_num = total_size / sizeof(Edge<T>);
        edge = new Edge<T>[g_e_num];
        fclose(f);
        size_t sum_ret = 0;
	printf("meant to read %lu edges\n",g_e_num);

#pragma omp parallel num_threads(partition_num)
        {
        	size_t l_e_num = g_e_num / partition_num;
                int partition_id = omp_get_thread_num();
                FILE * lf = fopen(fname,"rb");

                if(partition_id == partition_num -1)
                {
                        l_e_num += g_e_num % partition_num;
                }
                size_t f_offset = sizeof(Edge<T>) * (g_e_num / partition_num) * partition_id;
                fseek(lf, f_offset, SEEK_SET);
                auto ret = fread(edge, sizeof(Edge<T>),l_e_num, lf);
                #pragma omp atomic
                sum_ret = sum_ret + ret;
                fclose(lf);
        }
	printf("actually read %lu \n",sum_ret);
        assert(sum_ret == g_e_num);
        if(sum_ret != g_e_num){
...
void partition_relabel(const char* graph_path,const char* test_edges_path, vertex_id_t v_num,int partition_num)
{
    ...
    prl_read_edge_by_bin(graph_path, graph_edges, graph_e_num,batch_num);

    Edge<EmptyData>* test_edges;
    size_t test_e_num;
    read_edge_by_txt(test_edges_path, test_edges, test_e_num);
    //prl_read_edge_by_bin(test_edges_path, test_edges, test_e_num,batch_num);
    ...
    FILE* f_graph_path_r = fopen(graph_path_r.c_str(), "wb");
    for (edge_id_t e_i = 0; e_i < graph_e_num; e_i++)
    {
        if(e_i%2==0)
        // fprintf(f_graph_path_r,"%u %u %d\n", graph_edges[e_i].src, graph_edges[e_i].dst, graph_edges[e_i].data);
	fwrite(&graph_edges[e_i],sizeof(Edge<T>),1,f_graph_path_r);
    }
...
int main(int argc, char* argv[])
{
    ...
    stream_seq = opt.stream;
    if(stream_seq != "bfs" && stream_seq != "dfs")
    {
        printf("[ error ] stream sequence \n");
        exit(1);
    }
